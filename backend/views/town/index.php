...
/* @var $searchModel backend\models\search\TownSearch */
...
$columns = [
    ...
    [
        ...
        'format' => 'raw',
        'filter' => ArrayHelper::map(District::find()->all(), 'id', 'name'),
...
<div class="modal fade create-update-town" tabindex="-1" role="dialog"
...
<div class="town-index">
    ...
    <?= GridView::widget([
        ...
        'toolbar' => [
            ...
            Html::button('<i class="fa fa-lg fa-plus"></i>', ['class' => 'btn pl-2 pr-2 btn-success create-town']) .
...
<script type="text/javascript">
    ...
    $('body').on('click', '.update-town', function () {
        var id = $(this).data('id');
        $.ajax({
            type: 'get',
            url: '<?= yii\helpers\Url::to(["town/update"]) ?>',
            data: {id: id},
            success: function (res) {
                $('#create-update-form').html(res);
                $('.create-update-town').modal('show');
            },
            error: function (e) {
                console.log(e);
            }
    ...
    $('.create-town').click(function () {
        $.ajax({
            type: 'get',
            url: '<?= yii\helpers\Url::to(["town/create"]) ?>',
            success: function (res) {
                $('#create-update-form').html(res);
                $('.create-update-town').modal('show');
            },
    ...
    $('body').on('click', '#save-town-form', function (e) {
        e.preventDefault();
        var form = $('#save-town');
        ...
        $.ajax({
            ...
            success: function (res) {
                if (res.success === 1) {
                    $('.create-update-town').modal('hide');
                    $('#create-update-form').html(res.view);

//                    $.pjax.reload({container: "#pjax-town"});
