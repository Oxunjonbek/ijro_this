...
$columns = [
    ...
    [
        'attribute' => 'grade_id',
        'format' => 'raw',
        'filter' => ArrayHelper::map(Grade::find()->all(), 'id', 'name'),
        ...
        'value' => function ($model) {
            $grade = Grade::findOne($model->grade_id);

            return Html::tag("p", $grade->name, [
    ...
    [
        ...
        'headerOptions' => ['class' => 'text-center'],
        'value' => function ($model) {
            return Html::tag("p", $model->full_name, [
                'class' => 'd-flex justify-content-center mt-3'
            ]);
        }
    ],
    ...
    [
        ...
        'headerOptions' => ['class' => 'text-center'],
        'value' => function ($model) {
            return Html::tag("p", $model->telephone, [
                'class' => 'd-flex justify-content-center mt-3'
            ]);
        }
    ],
    ...
    [
        'attribute' => 'image',
        'format' => 'raw',
        'filter' => ArrayHelper::map(Pupil::find()->all(), 'id', 'image'),
        'filterType' => GridView::FILTER_SELECT2,
        'headerOptions' => ['class' => 'text-center'],
        'filterWidgetOptions' =>
            [
        ...
        'value' => function ($model) {
            return Html::img(
                $model->getImage(),
                ['height' => 100, 'width' => 100, 'class' => 'rounded mx-auto d-block']
            );
        }
    ],
    [
        ...
        'buttons' => [
            'Parent' => function ($url, $model) {
                ...
                return Html::a('<button class="btn btn-success">Ota-Onalar</button>', ['/parent/parent-from', 'pupil_id' => $model->id]
                );
            },
...
<div class="modal fade create-update-pupil" tabindex="-1" role="dialog"
...
<div class="pupil-index">
    ...
    <?= GridView::widget([
        ...
        'toolbar' => [
            ...
            Html::button('<i class="fa fa-lg fa-plus"></i>', ['class' => 'btn pl-2 pr-2 btn-success create-pupil']) .
...
<script type="text/javascript">
    <?php ob_start() ?>
    $('body').on('click', '.update-pupil', function () {
        var id = $(this).data('id');
        $.ajax({
            type: 'get',
            url: '<?= yii\helpers\Url::to(["pupil/update"]) ?>',
            data: {id: id},
            success: function (res) {
                $('#create-update-form').html(res);
                $('.create-update-pupil').modal('show');
            },
            error: function (e) {
                console.log(e);
            }
    ...
    $('.create-pupil').click(function () {
        $.ajax({
            type: 'get',
            url: '<?= yii\helpers\Url::to(["pupil/create"]) ?>',
            success: function (res) {
                $('#create-update-form').html(res);
                $('.create-update-pupil').modal('show');
            },
    ...
    $('body').on('click', '#save-pupil-form', function (e) {
        e.preventDefault();
        var form = $('#save-pupil');
        ...
        $.ajax({
            ...
            success: function (res) {
                if (res.success === 1) {
                    $('.create-update-pupil').modal('hide');
